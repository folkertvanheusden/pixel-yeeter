...
class frontend:
    ...
    # r/g/b: 0...255
    def set_pixel_rgb(self, x: int, y: int, r: int, g: int, b: int, a: int = 255, overlay: bool = False) -> None:
        self.b.set_pixel_alpha(x, y, r, g, b, a, overlay)

    def draw_line_rgb(self, x_start: int, y_start: int, x_end: int, y_end: int, r: int, g: int, b: int, a: int = 255, overlay: bool = False) -> None:
        dx = x_end - x_start
        dy = y_end - y_start
        steep = abs(dy) > abs(dx)
        if steep:
        ...
        for x in range(x_start, x_end + 1):
            ...
            self.b.set_pixel_alpha(coord[0], coord[1], r, g, b, a, overlay)
            error -= abs(dy)
            if error < 0:
                y += ystep
                error += dx

    def draw_line_color_by_name(self, x_start: int, y_start: int, x_end: int, y_end: int, color: str, overlay: bool = False) -> None:
        r, g, b, a = self.color_name_to_rgb_alpha(color)
        self.draw_line_rgb(x_start, y_start, x_end, y_end, r, g, b, a, overlay)

    def draw_pil_Image(self, pil_canvas: Image, x_offset: int, y_offset: int, overlay: bool = False):
    ...
    def draw_text(self, x: int, y: int, font_name: str, font_height: float, text: str, r: int, g: int, b: int, overlay: bool = False) -> None:
        ...
        self.draw_pil_Image(image, x, y, overlay)
    ...
    def fill_region_color_by_name(self, x: int, y: int, width: int, height: int, color: str, overlay: bool = False) -> None:
        r, g, b, a = self.color_name_to_rgb_alpha(color)
        self.fill_region_rgb(x, y, width, height, r, g, b, a, overlay)
...
class scroll_text(animation):
    ...
    def tick(self, f: frontend) -> None:
        ...
        if self.clock == 10:
            self.clock = 0
            f.clear_overlay()
            f.draw_pil_Image(self.image, self.x, 0, overlay = True)
            self.x -= 1
